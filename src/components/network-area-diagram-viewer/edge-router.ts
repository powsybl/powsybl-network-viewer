...
export class EdgeRouter {
    ...
    threeWtEdgeMap: Map<string, [Point, Point]> = new Map<string, [Point, Point]>();
    ...
    public getThreeWtEdgePoints(edgeId: string): Point[] | undefined {
        const points = this.threeWtEdgeMap.get(edgeId);
    ...
    private groupEdges(): {
        groupedEdges: Map<string, EdgeMetadata[]>;
        loopEdges: Map<string, EdgeMetadata[]>;
        threeWtEdges: Map<string, EdgeMetadata[]>;
    } {
        ...
        const threeWtEdges: Map<string, EdgeMetadata[]> = new Map<string, EdgeMetadata[]>();
        this.diagramMetadata.edges.forEach((edge) => {
            let edgeGroup: EdgeMetadata[] = [];
            if (MetadataUtils.isThreeWtEdge(edge)) {
                if (threeWtEdges.has(edge.node2)) {
                    edgeGroup = threeWtEdges.get(edge.node2) ?? [];
                }
                edgeGroup.push(edge);
                threeWtEdges.set(edge.node2, edgeGroup);
            } else if (edge.node1 == edge.node2) {
        ...
        return { groupedEdges: groupedEdges, loopEdges: loopEdges, threeWtEdges: threeWtEdges };
    }
    ...
    private storeThreeWtEdges(edgesMap: Map<string, EdgeMetadata[]>) {
        for (const threeWtId of edgesMap.keys()) {
            const threeWtNode = MetadataUtils.getNodeMetadata(threeWtId, this.diagramMetadata);
            if (!threeWtNode) {
                continue;
            }
            const threeWtEdges: EdgeMetadata[] = edgesMap.get(threeWtId) ?? [];
    ...
    private storeThreeWtEdge(
        ...
        this.threeWtEdgeMap.set(edge.svgId, [edgeStart, threeWtAnchor]);
    }
}
