...
export class NetworkAreaDiagramViewer {
  ...
  constructor(
    ...
    this.dynamicCssRules = cloneRules(
    ...
    this.layoutParameters = new LayoutParameters(
      diagramMetadata?.layoutParameters
    );
  ...
  private getTextNodeIdFromEquipmentId(equipmentId: string) {
    const node: TextNodeMetadata | undefined =
      this.diagramMetadata?.textNodes.find(
        (node) => node.equipmentId == equipmentId
      );
    return node?.svgId || null;
  }
  ...
  public moveTextNodeToCoordinates(
    ...
    const nodeElement: SVGGraphicsElement | null = this.svgDiv.querySelector(
      '[id="' + nodeId + '"]'
    );
    if (!nodeElement) {
      return;
    ...
    this.endTextEdge = new Point(
      nodePosition.x + connectionShiftX,
      nodePosition.y + connectionShiftY
    );

    const textNodeTopLeftCornerPosition = new Point(
      nodePosition.x + shiftX,
      nodePosition.y + shiftY
    );
    const textNodeCenterPosition =
      DiagramUtils.getTextNodeCenterFromTopLeftCorner(
        elemToMove,
  ...
  public init(
    ...
    const dimensions: DiagramUtils.Dimensions | null =
      this.getDimensionsFromSvg();
    if (!dimensions) {
      return;
    }
    ...
    if (this.onRightClickCallback != null && hasMetadata) {
      ...
      const emptyElements: NodeListOf<SVGGraphicsElement> =
        this.svgDiv.querySelectorAll(".nad-branch-edges .nad-winding");
      emptyElements.forEach((emptyElement) => {
        emptyElement.style.fill = "#0000";
      });
    }
  ...
  private onMouseLeftDown(event: MouseEvent) {
    ...
    if (event.shiftKey) {
      // selecting node
      this.onSelectStart(
        DiagramUtils.getSelectableFrom(event.target as SVGElement)
      );
    } else {
      // moving node
      this.onDragStart(
        DiagramUtils.getDraggableFrom(event.target as SVGElement)
      );
    }
  }
  ...
  private onDragStart(draggableElem: SVGElement | undefined) {
    ...
    if (this.textNodeSelected) {
      ...
      const textNode: TextNodeMetadata | undefined =
        this.diagramMetadata?.textNodes.find(
          (textNode) => textNode.svgId == this.draggedElement?.id
        );
      if (textNode) {
        this.originalTextNodeShift = new Point(
          textNode.shiftX,
          textNode.shiftY
        );
        this.originalTextNodeConnectionShift = new Point(
  ...
  private updateTextNodeMetadata(
    textNodeElement: SVGGraphicsElement,
    position: Point
  ) {
    const node: NodeMetadata | undefined = this.diagramMetadata?.nodes.find(
      (node) =>
        node.svgId == DiagramUtils.getVoltageLevelNodeId(textNodeElement.id)
    );
    const textNode: TextNodeMetadata | undefined =
      this.diagramMetadata?.textNodes.find(
    ...
    if (node != null && textNode != null) {
      const textNodeMoves = DiagramUtils.getTextNodeMoves(
        textNode,
        node,
        position,
        this.endTextEdge
  ...
  private onHover(mouseEvent: MouseEvent) {
    ...
    const hoverableElem = DiagramUtils.getHoverableFrom(
      mouseEvent.target as SVGElement
    );
    if (!hoverableElem) {
      this.onToggleHoverCallback(false, null, "", "");
      return;
  ...
  private updateVoltageLevelText(
  ...
  private updateNodePosition(vlNode: SVGGraphicsElement, position: Point) {
    vlNode.setAttribute(
      "transform",
      "translate(" + DiagramUtils.getFormattedPoint(position) + ")"
    );
  }
  ...
  private updateTextNodePosition(
  ...
  private redrawTextEdge(
    ...
    const textEdge: SVGGraphicsElement | null = this.svgDiv.querySelector(
      "[id='" + textEdgeId + "']"
    );
    if (textEdge != null) {
      // compute voltage level circle radius
      const busNodes: BusNodeMetadata[] | undefined =
        this.diagramMetadata?.busNodes.filter(
          (busNode) => busNode.vlNode == vlNode.id
        );
      const nbNeighbours =
        busNodes !== undefined && busNodes.length > 1 ? busNodes.length - 1 : 0;
  ...
  private updateEdges(vlNode: SVGGraphicsElement, position: Point) {
    ...
    const edges: EdgeMetadata[] | undefined =
    ...
    const groupedEdges: Map<string, EdgeMetadata[]> = new Map<
      string,
      EdgeMetadata[]
    >();
    const loopEdges: Map<string, EdgeMetadata[]> = new Map<
      string,
      EdgeMetadata[]
    >();
    const busNodeEdges: Map<string, EdgeMetadata[]> = new Map<
      string,
      EdgeMetadata[]
    >();
    edges?.forEach((edge) => {
      ...
      if (edge.node1 == edge.node2) {
        ...
        const busNodeId =
          edge.node1 == vlNode.id ? edge.busNode1 : edge.busNode2;
  ...
  private getEdgeNodes(
    ...
    const otherNode: SVGGraphicsElement | null = this.svgDiv.querySelector(
      "[id='" + otherNodeId + "']"
    );
  ...
  private getNodeRadius(
    ...
    const busNode: BusNodeMetadata | undefined =
      this.diagramMetadata?.busNodes.find(
  ...
  private redrawForkEdge(edges: EdgeMetadata[], vlNode: SVGGraphicsElement) {
    ...
    edges.forEach((edge) => {
      if (2 * i + 1 == nbForks) {
        ...
        // get edge element
        const edgeNode: SVGGraphicsElement | null = this.svgDiv.querySelector(
          "[id='" + edge.svgId + "']"
        );
        if (!edgeNode) {
          return;
        ...
        const unknownBusNode1 =
          edge.busNode1 != null && edge.busNode1.length == 0;
        ...
        const unknownBusNode2 =
          edge.busNode2 != null && edge.busNode2.length == 0;
    ...
    const otherNode: SVGGraphicsElement | null = this.getOtherNode(
      edgeNodes,
      vlNode
    );
  ...
  private getEdgeStart(
    ...
    return DiagramUtils.getPointAtDistance(
      ...
      unknownBusNode
        ? outerRadius + this.svgParameters.getUnknownBusNodeExtraRadius()
        : outerRadius
    );
  }
    ...
    if (positionElement != null) {
      ...
      if (angleElement != null) {
        ...
        angleElement.setAttribute(
          "transform",
          "rotate(" + DiagramUtils.getFormattedValue(edgeNameAngle) + ")"
        );
      }
    }
  }
  ...
  private getEdgeAngle(
    ...
    const halfEdgeId =
      busNode.svgId == edge.busNode1 ? edgeId + ".1" : edgeId + ".2";
    if (!this.edgeAngles.has(halfEdgeId)) {
      ...
      const halfEdgeDrawElement: HTMLElement | null = <HTMLElement>(
        (this.svgDiv
          .querySelector("[id='" + halfEdgeId + "']")
          ?.querySelector("path, polyline") as Element)
      );
  ...
  private redrawBusNode(
    ...
    const busElement: HTMLElement | null = <HTMLElement>(
      node.querySelectorAll(".nad-busnode")[busIndex]
    );
    if (busElement != null) {
  ...
  private redrawOtherVoltageLevelNode(otherNode: SVGGraphicsElement | null) {
    if (otherNode != null) {
      ...
      const edges: EdgeMetadata[] | undefined =
        this.diagramMetadata?.edges.filter(
          (edge) =>
            edge.node1 == (otherNode?.id ?? -1) ||
            edge.node2 == (otherNode?.id ?? -1)
        );
      ...
      edges?.forEach((edge) => {
        if (edge.node1 == edge.node2) {
          ...
          const busNodeId =
            edge.node1 == otherNode?.id ? edge.busNode1 : edge.busNode2;
          this.addBusNodeEdge(busNodeId, edge, busNodeEdges);
        }
      });
      // redraw other voltage level node
  ...
  public checkAndUpdateLevelOfDetail(svg: SVGSVGElement) {
    ...
    if (
      Math.abs(previousMaxDisplayedSize - maxDisplayedSize) /
        previousMaxDisplayedSize <
  ...
  public setBranchStates(branchStates: BranchState[]) {
    branchStates.forEach((branchState) => {
      ...
      if (branchState.connectedBus1 && edge) {
        this.setBranchBusConnection(
          edge,
          branchState.branchId,
          "1",
          branchState.connectedBus1
        );
      }
    ...
    if (arrowGElement !== null) {
      ...
      if (branchLabelElement !== null) {
        branchLabelElement.innerHTML =
          typeof value === "number"
            ? value.toFixed(this.getValuePrecision())
            : value;
      } else {
        console.warn(
          "Skipping updating branch " +
  ...
  private setBranchSideConnection(
    branchId: string,
    side: string,
    edgeId: string,
    connected: boolean | undefined
  ) {
    const halfEdge: SVGGraphicsElement | null = this.svgDiv.querySelector(
      "[id='" + edgeId + "." + side + "']"
    );
    if (halfEdge !== null) {
   ...
   */
  private setBranchBusConnection(
    edge: EdgeMetadata,
    branchId: string,
    side: string,
    busId: string
  ) {
    const targetBusNode = this.diagramMetadata?.busNodes.find(
      (busNode) => busNode.equipmentId === busId
    );
    if (!targetBusNode) {
      console.warn(
        "Skipping updating branch " +
    ...
    const currentBusNode = this.diagramMetadata?.busNodes.find(
      (busNode) => busNode.svgId === currentBusNodeId
    );
    ...
    const vlElement = this.container.querySelector(
      `[id='${targetBusNode.vlNode}']`
    ) as SVGGraphicsElement;
    if (!vlElement) {
  ...
  private addStyle() {
    ...
    if (
      this.svgParameters.getCssLocation() == CssLocationEnum.EXTERNAL_NO_IMPORT
    ) {
      const styleElement = DiagramUtils.getStyle(
  ...
  private removeStyle() {
    ...
    if (
      ...
      const styleElement: HTMLElement | null =
        this.svgDiv.querySelector("style");
      styleElement?.remove();
    }
  }
