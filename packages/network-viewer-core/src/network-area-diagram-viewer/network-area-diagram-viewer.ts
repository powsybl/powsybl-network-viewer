...
export class NetworkAreaDiagramViewer {
    ...
    private redrawBranchEdge(edge: EdgeMetadata, halfEdge1: HalfEdge | null, halfEdge2: HalfEdge | null) {
        ...
        if (edge.edgeInfoMiddle) {
            ...
            this.redrawMiddleEdgeArrowAndLabels(
                ...
                halfEdge2,
                edgeInfoElement,
                edge.edgeInfoMiddle.direction ?? edge.edgeInfoMiddle.directionB,
                edge.edgeInfoMiddle.directionA,
                hasBothLabels
            );
        }
    }
    ...
    private redrawEdgeArrowAndLabels(halfEdge: HalfEdge, edgeInfo: SVGElement | null = null) {
        ...
        // move edge info
        const infoPoint = HalfEdgeUtils.getInfoPoint(halfEdge, this.svgParameters);
        ...
        this.redrawLabel(labelBElement, labelData.angle, labelData.external.shift * factor, labelData.external.style);
        // move edge labelA
    ...
    private redrawMiddleEdgeArrowAndLabels(
        ...
        let i = 1;
        if (bothLabels) {
            const labelBElement = edgeInfo.querySelector('text:nth-of-type(' + i++ + ')');
        ...
        const labelAElement = edgeInfo.querySelector('text:nth-of-type(' + i + ')');
        if (labelAElement) {
    ...
    private adaptiveZoomViewboxUpdate(maxDisplayedSize: number) {
        ...
        if (maxDisplayedSize > adaptiveTextZoom.threshold) {
            ...
            if (maxDisplayedSize > adaptiveTextZoom.edgeSideLabelThreshold) {
                for (const edge of containedEdgeList) {
                    if (edge.edgeInfo1) {
            ...
            start = performance.now();
            this.createEdgesInfos(containedEdgeList, maxDisplayedSize);
    ...
    private addBranchArrowElement(
        ...
        if (arrowPath) {
            const edgeInfoArrow = this.getOrCreateEdgeInfoArrow(edgeInfo, i);
            ...
            if (edgeInfoDirectionClass) {
                edgeInfoArrow.classList.add(edgeInfoDirectionClass);
            }
            const edgeInfoTypeClass = DiagramUtils.getEdgeInfoTypeClass(type);
            if (edgeInfoTypeClass) {
                edgeInfoArrow.classList.add(edgeInfoTypeClass);
            }
        }
    }
    ...
    private getOrCreateEdgeInfoArrow(edgeInfo: SVGElement, i: number): SVGPathElement {
        let edgeInfoArrow = edgeInfo.querySelector('path:nth-of-type(' + i + ')') as SVGPathElement;
        if (!edgeInfoArrow) {
